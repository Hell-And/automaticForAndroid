 ...
 */
public class DouyinFastFollowMonitor {
    private static final DouyinFastFollowMonitor instance = new DouyinFastFollowMonitor();
    private SuperServiceMonitor superServiceMonitor;
    private AccessibilityNodeInfo nodeInfo;

    public static DouyinFastFollowMonitor getInstance() {
        return instance;
    }
...
//        Log.d("GestureDescription", result + "");
    ...
    private static void forceMove(SuperServiceMonitor accessibilityServiceMonitor, AccessibilityNodeInfo nodeInfo) {
        List<AccessibilityNodeInfo> tagTuijian=nodeInfo.findAccessibilityNodeInfosByText("推荐");
        List<AccessibilityNodeInfo> tagAction=nodeInfo.findAccessibilityNodeInfosByViewId("com.ss.android.ugc.aweme.lite:id/bkn");
        if (tagTuijian.size() <= 0 || tagAction.size() <= 0){
            return;
        }
        int width = AccessibilitUtil.getScreenInfo().get("width");
        int height = AccessibilitUtil.getScreenInfo().get("height");
        GestureDescriptionUtil.moveMonitor(accessibilityServiceMonitor,width / 2, height - 100,width / 2, 100,null);
    }
    private int followThisStart = 0;
    public void getAcType(SuperServiceMonitor accessibilityServiceMonitor, AccessibilityNodeInfo nodeInfo) {
        if (nodeInfo != null) {
            ...
            if (list.isEmpty() && followThisStart < 0) {
